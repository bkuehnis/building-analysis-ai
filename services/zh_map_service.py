...
class ZhMapService:
    ...
    def fetch_zh_map_screenshot(self, x, y, address, scale=400, output_dir="output/images"):
        """Fetch screenshot of Zürich map at given coordinates (map + ortho)"""

        os.makedirs(output_dir, exist_ok=True)

        safe_address = address.replace("/", "-").replace(" ", "_")

        filename = f"{output_dir}/zh_map.png"
        filename_ortho = f"{output_dir}/zh_map_ortho.png"

        url = f"https://geo.zh.ch/maps?x={x}&y={y}&scale={scale}&basemap=areavbackgroundzh"
        url_ortho = f"https://geo.zh.ch/maps?x={x}&y={y}&scale={scale}&basemap=areorthobackgroundzh"
        ...
        with sync_playwright() as p:
            ...
            browser.close()
        return filename, filename_ortho